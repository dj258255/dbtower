...
class Settings:
    ...
    # poll_once가 한 배치를 끝까지 기다린 뒤 다음 메시지를 읽으므로(170절 7번) 그동안 큐가 밀린다.
    #
    # 두 값 모두 플랫폼의 dbtower.aiops.lease-seconds(기본 300)보다 작아야 한다. 크게 잡으면 실행기가 아직
    # 응답을 기다리는 사이 리퍼가 리스 만료로 작업을 닫고, 그 뒤 실행기의 호출은 409로 돌아온다. 409는 "다른
    # 실행기가 가져갔다"로 보고 메시지를 확인 처리하므로 재배달이 없고, 실패 알림도 나가지 않는다(170절 2번과 같은 침묵).
