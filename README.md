...
Oracle은 샘플에 부모·자식 쌍이 없어(후보 열의 고아 행 14,000건) 데모 열을 티켓으로 더한 뒤 외래키를 걸고 되돌렸습니다. 이로써 외래키 수집·비교·실행 기록이 5기종 모두 라이브로 확인됐습니다([VERIFICATION 157절](docs/VERIFICATION.md)).
...
쿼리 상세에는 안티패턴 신호를 더했습니다. 느림의 크기가 아니라 성질(인덱스 없이 훑는가, 디스크로 넘치는가, 한 행에 얼마나 읽는가)을 통계 뷰만으로 읽어,
실행계획을 뜨기 전에 먼저 볼 쿼리를 좁힙니다. 축은 5기종 공통이고 원천 지표 이름·단위는 기종이 답하며, 그 기종에 없는 축은 0이 아니라 미확보로 적습니다([VERIFICATION 158절](docs/VERIFICATION.md)).
