...
![MCP 연동 카드 — 도구 9종은 실시간 tools/list 응답](docs/images/webui/06-mcp.png)
...
Grafana/PMM 처럼 자기 인프라에 직접 띄워 자기 DB 를 붙이는 모델이다. 관리 "도구"(앱 + 메타 DB)만
띄우고, 관리 "대상" DB 는 웹 UI 에서 등록한다. 이미지에 mysqldump/pg_dump/mongodump 가 번들되어
있어(배터리 포함) 백업까지 추가 설치 없이 동작한다.
...
이미지는 [GHCR](https://github.com/dj258255/dbtower/pkgs/container/dbtower) 에서 pull 하거나
(`ghcr.io/dj258255/dbtower:latest`), 위 compose 가 `--build` 로 로컬 빌드한다. 릴리스는 `vX.Y.Z`
태그를 push 하면 `release.yml` 워크플로가 이미지를 게시한다.
