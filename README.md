...
| 로그 백업·PITR | 5기종 로그 백업(binlog·WAL·BACKUP LOG·oplog·아카이브) + 물리 앵커(pg_basebackup·RMAN) + 복원 가능 창·문안 생성, MSSQL·PG 실복원 e2e |
| 디스크 포화 예측 | "지금 여유"가 아니라 "이 속도면 며칠 뒤 차는가" — node_exporter 선형 추세 ETA(3일 치명/14일 경고), 인스턴스-노드 매핑(nodeFilter) |
