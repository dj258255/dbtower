// 스키마 트리 — 클릭은 이름을 편집기 커서에, 더블클릭은 미리보기 쿼리를 새 탭에.
...
export function renderTree(container, schema, { filter, expanded, onInsert, onPreview, onToggle }) {
  if (!schema) {
  ...
  container.innerHTML = cap + `<ul class="tree">${tables.map((t) => {
    ...
    const columns = open
      ? `<ul class="tree-cols">${t.columns.map((c) =>
        `<li class="tree-col" data-insert="${esc(c.name)}" title="${esc(c.type)}${c.nullable ? "" : " NOT NULL"}"><span>${esc(c.name)}</span><span class="muted">${esc(c.type)}</span></li>`
      ).join("")}</ul>`
      : "";
    return `<li class="tree-table">
      <div class="tree-row">
        <button class="tree-caret" data-toggle="${esc(t.name)}" aria-label="열 펼치기">${open ? "▾" : "▸"}</button>
        <span class="tree-name" data-insert="${esc(t.name)}" data-preview="${esc(t.name)}" title="클릭: 이름 넣기 · 더블클릭: 미리보기 쿼리">${esc(t.name)}</span>
