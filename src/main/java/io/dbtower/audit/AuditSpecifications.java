...
final class AuditSpecifications {
    ...
    static Specification<AuditEvent> principalIs(String principal) {
    ...
    static Specification<AuditEvent> actionContains(String action) {
        return isBlank(action) ? null
                : (root, query, cb) -> cb.like(root.get("action"), "%" + action + "%");
    }

    static Specification<AuditEvent> instanceIdIs(Long instanceId) {
    ...
    static Specification<AuditEvent> outcomeIs(Integer outcome) {
        return outcome == null ? null
                : (root, query, cb) -> cb.equal(root.get("outcome"), outcome);
    }

    static Specification<AuditEvent> occurredFrom(LocalDateTime from) {
        return from == null ? null
                : (root, query, cb) -> cb.greaterThanOrEqualTo(root.get("occurredAt"), from);
    }

    static Specification<AuditEvent> occurredTo(LocalDateTime to) {
        return to == null ? null
