    ...
    private void registerTools() {
        ...
        tools.put("lakehouse_query", new Tool(
                "장기 마트 SELECT 질의(수개월 이력 — 7일 시야 도구들이 못 답하는 추세·분기 비교). "
                        ...
                        + "mart_wait_top(instance_id, instance_name, engine, rank_in_instance, event_name, category, "
                        + "total_delta_ms, sum_last_ms — 인스턴스별 상위 대기, 기종으로 delta/last 의미 해석), "
                        ...
                        + "mart_weekly_ops_report(week_start, is_partial_week, instance_id, instance_name, engine, "
                        + "capacity_worst_risk, min_days_to_threshold, top_wait_event, plan_flips_this_week, "
                        + "plan_regressed_this_week, backup_gap_days, backup_status, this_week_uptime_pct — "
                        + "인스턴스별 주간 요약 한 행), "
                        + "fct_uptime_daily(instance_id, dt, samples, up_samples, uptime_pct, avg_ping_ms, p95_ping_ms), "
                        + "mart_uptime_slo(instance_id, instance_name, engine, window_uptime_pct, worst_day_uptime_pct, "
                        + "target_pct, error_budget_remaining_pct, slo_status — meets/at_risk/breach, 장기 가용성 SLO), "
                        ...
                        + "regressed_after, before/after_latency_ms, latency_ratio, before/after_bytes, size_ratio, "
                        + "correlation — followed_by_regression/plan_flip/latency_rise/size_growth/no_signal, 변경 뒤 "
                        + "회귀 상관·상관≠인과), "
                        + "dim_instance(instance_id, instance_name, engine — MYSQL/POSTGRESQL/MSSQL/MONGODB/ORACLE, "
                        + "team_label, 기종 축 차원 — instance_id만 있는 마트에 조인해 기종·이름을 붙인다. "
                        + "mart_wait_top·mart_weekly_ops_report엔 이미 engine·instance_name 편입됨), "
