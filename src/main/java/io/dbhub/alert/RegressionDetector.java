...
import io.dbhub.insight.ComparisonService;
import io.dbhub.insight.QueryDiff;
...
public class RegressionDetector {
    ...
    public RegressionDetector(DatabaseInstanceRepository instanceRepository,
                              ...
                              @Value("${dbhub.regression.recent-minutes:5}") int recentMinutes,
                              @Value("${dbhub.regression.baseline-minutes:15}") int baselineMinutes,
                              @Value("${dbhub.regression.cooldown-minutes:30}") int cooldownMinutes) {
    ...
    @Scheduled(fixedDelayString = "${dbhub.regression.poll-ms:120000}")
    public void detect() {
    ...
    private void notify(DatabaseInstance instance, List<String> findings) {
        ...
        message.append("[DBHub 회귀 감지] instance=").append(instance.getName())
