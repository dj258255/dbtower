...
class SnapshotRetentionAdvisorTest {
    ...
    private DatabaseInstance instance() {
        // id는 리플렉션 없이 지정하기 어려워 batchCount가 어떤 id로 불려도 되게 any()로 스텁
        return new DatabaseInstance("db1", DbmsType.MYSQL, "h", 3306, "sample", "u", "p");
    }

    @Test
    void 보존이_꺼져있고_스냅샷이_쌓여있으면_경고() {
        when(comparisonService.batchCount(any(), any(), any())).thenReturn(42);
        SnapshotRetentionAdvisor advisor = new SnapshotRetentionAdvisor(comparisonService, 0);
    ...
    @Test
    void 보존이_켜져있으면_통과() {
        SnapshotRetentionAdvisor advisor = new SnapshotRetentionAdvisor(comparisonService, 7);
        assertTrue(advisor.inspect(instance(), operator).isEmpty());
        Mockito.verifyNoInteractions(comparisonService);
    }

    @Test
    void 보존이_꺼져도_아직_수집된_스냅샷이_없으면_통과() {
        when(comparisonService.batchCount(any(), any(), any())).thenReturn(0);
        SnapshotRetentionAdvisor advisor = new SnapshotRetentionAdvisor(comparisonService, -1);
        assertTrue(advisor.inspect(instance(), operator).isEmpty());
    }
