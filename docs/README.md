...
| 폴더 | 무엇이 있나 |
|---|---|
| [work/](work/) | **작업 관리** — 로드맵, 이슈·PR·릴리스·협업 규칙과 그렇게 하는 이유 |
| [design/](design/) | 아키텍처와 판단 기준, 착수 전에 정한 명세 |
| [operate/](operate/) | 설치·운영·최소 권한·API |
| [verify/](verify/) | 돌린 명령과 출력 원문, 역할별 수동 테스트 |
| [portfolio/](portfolio/) | 한 장 요약, 발표, 기술 질문과 답, 시연 순서, 화면 |
| [archive/](archive/) | 끝난 감사·명세. 결정의 이유를 찾을 때만 읽는다 |
| [experiments/](experiments/) | 실험 원자료와 판정 |
...
| 2 | [least-privilege.md](operate/least-privilege.md) | DBMS별 모니터링·조회·변경 계정 최소 권한 |
| 3 | [operations.md](operate/operations.md) | 수집·통계·알림, 정식 이미지 배포·업그레이드 시 주의점 |
| 4 | 인프라 자동 등록 | [Kubernetes](../infra/k8s/README.md) · [Terraform](../infra/terraform/README.md) · [Ansible](../infra/ansible/README.md) |
| 5 | [API.md](operate/API.md) | 토큰 인증, 주요 REST·MCP 진입점 |

### 온콜 — 경보를 받고 원인을 찾는 사람

| 순서 | 문서 | 여기서 얻는 것 |
|---|---|---|
| 1 | [DEMO-5MIN.md](portfolio/DEMO-5MIN.md) | Top Query -> 쿼리 상세(실행계획·AI 분석) -> 인덱스 제안 -> 변경 티켓 승인·실행까지 실제로 누르는 순서 |
| 2 | [ai-analysis-rules.md](design/ai-analysis-rules.md) | AI 1차 분석이 무엇을 기준으로 판정하는지, 기종별 예외 |
| 3 | [AI-OPERATIONS-AUTOMATION.md](design/AI-OPERATIONS-AUTOMATION.md) | Slack·경보가 만든 AI 작업이 어떤 사실로 어떤 소견을 내는지, 믿으면 안 되는 자리 |
| 4 | [SCREENSHOTS.md](portfolio/SCREENSHOTS.md) | 지금 화면이 어떻게 생겼는지 |
...
| 3 | [DESIGN.md](design/DESIGN.md) · [modules/](modules/) | 현재 아키텍처와 핵심 결정, Modulith가 생성한 모듈 의존 |
| 4 | [MANUAL-TEST.md](verify/MANUAL-TEST.md) | 역할별 계정으로 변경 티켓 흐름을 손으로 확인하는 절차 |
| 5 | [VERIFICATION.md](verify/VERIFICATION.md) | 고치려는 기능의 과거 결함과 측정 — 아래 "최근 검증 기록"에서 절 번호를 찾는다 |
| 6 | [bulk-change-spec.md](design/bulk-change-spec.md) | 대량 일괄 변경의 실행 방식·안전 기준. 5기종 전부와, 담지 않기로 판정한 범위(여러 테이블·`_id` 타입이 섞인 컬렉션)까지 |

### 리뷰어·평가자 — 설계와 근거를 확인하는 사람

| 순서 | 문서 | 여기서 얻는 것 |
|---|---|---|
| 1 | [PORTFOLIO-ONEPAGER.md](portfolio/PORTFOLIO-ONEPAGER.md) | 문제, 해법, 대표 수치 한 장 |
| 2 | [TECHNICAL-QA.md](portfolio/TECHNICAL-QA.md) | 설계·안전·검증에 관한 질문과 답 |
...
| 4 | [CHANGELOG.md](../CHANGELOG.md) -> [work/](work/) | 릴리스별 변경, 그리고 작업을 어떻게 쪼개고 기록했는지 |
| 5 | [PRESENTATION.md](portfolio/PRESENTATION.md) · [PORTFOLIO-AX.md](portfolio/PORTFOLIO-AX.md) | 발표 원고, AI를 운영 플랫폼에 안전하게 붙인 사례 |
