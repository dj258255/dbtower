...
| 4 | 한 플랫폼, 사람별 입구: 역할 5개(관제·요청자·승인자·운영자·관리자 — 승인자와 운영자는 서로의 일을 못 함, RoleHierarchy·V40), `/api/me` 능력 기반 화면 분기, 역할별 첫 화면(저장된 요청 우선), 넘김 셋(쿼리 상세 -> 워크벤치, 인덱스 제안 -> 변경 요청, 실행 기록 -> 대시보드 전후 비교), 사용자·역할 API·카드 | 135절 |
