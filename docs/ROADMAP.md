...
| D5 | **완료(47절)** 파티션 조회 (Partition Inventory) | KDMS MCP 6기능 중 하나 | describeSchema()(B7) 확장, DbmsOperator | 기종별 파티션 목록·범위·크기 조회(MySQL/PG/Oracle 파티셔닝, Mongo는 샤딩/UNSUPPORTED). KDMS 갭 중 마지막 조각. **자동 관리(생성·삭제)는 범위 밖** — 조회만 | 파티션 있는 테이블로 목록 반환 라이브(없으면 빈 결과·UNSUPPORTED 정직) |
...
## 심화 아크 2 — 문의 채널 진단 맥락 강화 (B8 심화, 착수 명세)

> **구현 담당: Opus. 착수 명세(미착수).** B8(문의 채널)은 쿼리·플랜·규칙·AI·비고까지 보내지만 정작
...
### Phase 1 — 단일노드 프로덕션 하드닝 (한 명이 안전하게)

| 항목 | 상태 | 구현 명세 (Opus) | 검증 기준 |
|---|---|---|---|
| TLS(리버스 프록시) | 미구현 | 인앱 TLS 대신 프록시 종단이 정석(인증서 갱신을 앱에 안 넣음). (a) `application-docker.yml`에 `server.forward-headers-strategy: framework` — 프록시 뒤에서 scheme·host 보존. (b) 세션 쿠키 `server.servlet.session.cookie.secure`를 설정 키로(프록시 TLS 시 true). (c) docs/operations.md에 Caddy 2줄 예시(자동 인증서)와 nginx 예시 + `docker-compose.proxy.yml` 샘플(선택). **함정**: CSRF 쿠키(`withHttpOnlyFalse`, SecurityConfig.java:56)에 Secure 플래그 — CookieCsrfTokenRepository 커스터마이즈 필요 | 프록시 뒤 로그인·CSRF·리다이렉트 e2e, 쿠키에 Secure 플래그 실측 |
| 메타DB 자기백업 | 미구현 | `backup/internal/job/MetaBackupJob`(@Scheduled+@SchedulerLock) — 번들된 pg_dump로 메타DB 자신을 `{backup.dir}/meta/`에 덤프, `RemoteBackupStore.upload()` 재사용(오프사이트). 설정 `dbtower.meta-backup.{enabled:true, interval-hours:24, retention-count:7}`. 비밀번호는 PGPASSWORD 환경변수(argv 금지 원칙). docs/operations.md에 복원 절차 + "Flyway 마이그레이션 전 메타 백업" 권고. **함정**: 대상 DB 백업과 이력 테이블 분리(BackupRun에 섞지 않기 — 자기 백업은 별도 단순 보존) | 잡 실행 → 덤프 생성 → 빈 PG에 psql 복원 → 앱 기동 e2e. 원격 업로드 실측 |
| 로그인 rate-limit | 미구현 | 새 라이브러리 없이: `security/internal/LoginAttemptGuard` — `AbstractAuthenticationFailureEvent` 리스너(기존 audit 리스너와 병렬)로 username+IP별 실패 카운터(인메모리 Caffeine 불요 — ConcurrentHashMap+만료), N회(기본 10) 실패 시 M분(기본 15) 잠금, 폼 로그인 앞 필터에서 423/429 + 남은 시간. 설정 `dbtower.security.login-lock.{max-attempts, lock-minutes}`. **함정**: 프록시 뒤 IP는 X-Forwarded-For(위 forward-headers 전제), 단일노드 인메모리 한계는 Phase 3에서 세션 스토어와 함께 재검토로 정직 표기 | 실패 반복 → 잠금 → 시간 경과 해제 단위 테스트, 성공 로그인 시 카운터 리셋 |
| actuator·API 토큰 | 미구현 | (a) `/actuator/prometheus`에 선택 토큰: `dbtower.metrics.token` 설정 시 `Authorization: Bearer` 검사(스크레이프 설정 예시 병행), 미설정 시 현행 permitAll 유지 + 기동 WARN 1회 + operations.md 네트워크 격리 가이드. (b) `ApiTokenProvider.java:29` — 미설정 시 매 기동 랜덤 재생성으로 MCP 연동이 재시작마다 깨짐 → 최초 생성 토큰을 메타DB(플랫폼 설정 테이블, V11)에 저장해 재시작 생존. ADMIN이 콘솔에서 재발급 가능 | 재시작 후 토큰 불변 실측, 메트릭 토큰 설정 시 401/200 분기 |
| 로깅·커뮤니티 파일 | 미구현 | `logback-spring.xml` — 콘솔(현행 유지) + 프로필별 롤링 파일(옵션, `LOG_PATH` 시), JSON 인코더는 범위 밖. `CONTRIBUTING.md`(빌드·테스트·검증 규칙은 AGENTS.md 링크), `CODE_OF_CONDUCT.md`(Contributor Covenant), `.github/ISSUE_TEMPLATE/`(bug·feature 2종)+`PULL_REQUEST_TEMPLATE.md`. README에 "시스템 요구사항" 절(JDK 21, 메모리 기준, 지원 대상 DB 버전은 docker/ 시드 버전 기준으로 정직하게) | 파일 존재 + README 링크. 로그 롤링은 로컬 실측 |

### Phase 2 — 기능 심화 (제품 깊이)

| 항목 | 상태 | 구현 명세 (Opus) | 검증 기준 |
|---|---|---|---|
| 로그 백업 5기종 확장 | 부분 | 현재 MSSQL만 실동작(`MsSqlOperator.java:60` BACKUP LOG). 확장 축: **MySQL** — `FLUSH BINARY LOGS` 후 직전 binlog 파일 수집(binlog 활성은 대상 설정이므로 "켜져 있으면 쓴다" 게이트: `SHOW VARIABLES LIKE 'log_bin'` OFF면 UNSUPPORTED 사유 명시). **PG** — WAL 아카이빙은 archive_command 등 대상 서버 설정 필요 → 대상 설정 변경 금지 원칙상 자동화하지 않고, `pg_basebackup` 기반 물리 FULL(+`--wal-method=stream`)을 별도 타입으로 검토(이것도 replication 권한 게이트). **Oracle** — RMAN 없이는 정직 UNSUPPORTED 유지. **Mongo** — `--oplog` 옵션 백업(복제셋 게이트). 각 기종 지원/미지원과 사유를 BackupResult에 명시 | 게이트 ON 환경(docker/ 시드에 binlog 활성) 실측 + OFF 환경 UNSUPPORTED 사유 확인 |
| PITR(임의 시점 복구) | 미구현 | **범위 결정이 선행**: 완전한 PITR 오케스트레이션은 대상 서버 구성(아카이브 경로·restore_command)을 요구해 "대상을 바꾸지 않는다" 정체성과 충돌. 권장 범위 — (1) 로그체인 보유 시 "복원 가능 시점 범위" 표시(FULL 시각 ~ 마지막 LOG 시각), (2) 기종별 복원 명령 **생성·안내**(실행은 사람: MSSQL RESTORE ... STOPAT, MySQL mysqlbinlog --stop-datetime 파이프), (3) 복원 검증(A7)을 STOPAT 경로로 확장(MSSQL은 임시 DB에 실제 시점 복원 검증 가능). 자동 실행까지 갈지는 이 범위의 실사용 후 재결정 | MSSQL에서 FULL+LOG 체인 → 중간 시점 복원 검증 e2e(임시 DB). 안내 명령 문자열 단위 테스트 |
| 문의 테이블 구조 첨부 | 미구현 | **위 "심화 아크 2 — 문의 채널 진단 맥락 강화" 절이 완성 명세**(I1~I5: describeTables IN-바인딩, referencedTables 추출기, 인덱스 중심 요약+행수 "≈", 임베드→파일→딥링크, 사이트 패널). 그 절대로 구현 | 그 절의 검증 기준 그대로 |
| 데이터 마스킹 | 부분 | `analysis/QueryMasker`(순수 유틸) — SQL **리터럴만** 치환(문자열 '...'→'?', 숫자→?, 식별자·구조 보존 — 진단력 유지가 목적이므로 과도 마스킹 금지). 적용 지점은 외부 발신 직전 3곳: WebhookNotifier(회귀·문의), AiAnalyzer 프롬프트(선택 토글), MCP 응답(DiagnosisService). 설정 `dbtower.masking.enabled`(기본 true)·`mask-ai-prompt`(기본 false — AI 판정 정확도와 트레이드오프 명시). KDMS MCP 보안 3단계의 3(데이터 마스킹) 대응 | 리터럴 치환 단위 테스트(인젝션형 문자열·이스케이프 포함), 문의 embed 실측에서 리터럴 가려짐 확인 |
...
| 노드 간 수집 샤딩 | 미구현 | 현재 `@SchedulerLock(name="snapshot-collect")` 단일 락 = 노드 간 한 노드만 수집(수평 확장 불가). 설계: `dbtower.snapshot.shards`(기본 1 — 현행과 동일 동작) 도입, 틱마다 샤드 s∈[0,N)별로 `snapshot-collect-s` 락을 **각각** 시도해 획득한 샤드만 수집(`instanceId % N == s` 담당). 노드가 여럿이면 락이 자연 분산되고, 한 노드만 있으면 전 샤드를 그 노드가 잡는다(페일오버 유지). consistent hashing 불요 — 카운터 누적 차분이라 담당 이동이 데이터를 깨지 않는다(스냅샷 구간만 한 번 끊김, ComparisonService가 이미 리셋 내성). **함정**: lockAtLeastFor/MostFor를 샤드별로 동일 유지, 같은 인스턴스가 두 샤드에 안 걸리게 모듈로만 사용 | 2노드×shards=4에서 담당 분산 로그 실측, 1노드 페일오버(전 샤드 인수) 실측 |
| 메타DB 파티셔닝 | 미구현 | `query_snapshot`(최대 볼륨)·`health_sample`을 PG 선언적 월별 RANGE 파티션으로. **함정**: 기존 테이블의 파티션 전환은 Flyway 한 방이 불가(신 테이블 생성→복사→스왑) — V13에 단계 명시, 대량 데이터 시 잠금 최소화 절차. 보존 삭제(SnapshotRetentionJob의 DELETE)를 `DROP PARTITION`(즉시·블로트 없음)으로 교체, 파티션 자동 생성은 잡에서(월 +1 선생성). pg_partman 의존은 안 함 | EXPLAIN 파티션 프루닝 확인, DELETE 대비 보존 처리 시간 before/after 실측 |
| 호스트 그룹핑(서버 공유 인지) | 미구현 | 등록 단위는 DB(host·port·dbName, name만 unique) — 같은 서버에 DB 여러 개를 등록하면 **DB 스코프 지표는 정확히 갈라지지만**(PG는 `dbid=current_database()` 필터, MySQL은 dbName 조회·SCHEMA_NAME 인지) **서버 전역 지표**(MySQL wait event·parameters·replication, 커넥션 풀·헬스체크)가 인스턴스 수만큼 중복 수집·중복 경보된다. 설계: `host:port` 파생 서버 그룹(엔티티 추가 없이 계산 키로 시작) — 서버 전역 지표 수집·경보는 그룹당 1회 dedup, UI에 "같은 서버" 묶음 표시, DB 스코프 지표는 현행 유지. 디스크 예측(Phase 5)의 nodeFilter 공유와 같은 축 | 같은 서버에 2 DB 등록 → 서버 전역 경보 1건으로 dedup 실측, DB 스코프 통계는 각각 유지 확인 |
| 커넥션 온디맨드 | 미구현 | `ConnectionPools.java:85` minimumIdle=1 → 0, `idleTimeout`(기본 10분)·`maxLifetime` 명시 + 장기 미사용 인스턴스 풀 자체를 close하는 LRU sweep(마지막 사용 시각 기록, `dbtower.pool.evict-after-minutes`). **함정**: 수집 주기(60s)보다 idleTimeout이 짧으면 매 틱 재연결 폭탄 — 하한 가드. 성능 주장은 실측 필수(AGENTS) — 인스턴스 N에서 유휴 FD/커넥션 수와 틱 수집 시간 before/after | 유휴 상태 커넥션 수 실측(before: N개 상시 / after: 0 수렴), 수집 지연 회귀 없음 확인 |

### Phase 5 — 분석 루프 (lakehouse 연동)

| 항목 | 상태 | 구현 명세 (Opus) | 검증 기준 |
|---|---|---|---|
| forward: offload 확대 | 미구현 | **DBTower 코드 변경 0** — lakehouse가 메타DB를 직접 추출하므로 dbtower-lakehouse 저장소 작업: extract 대상에 `wait event 스냅샷·backup_run·plan_snapshot` 테이블 추가(각각 스키마·파티션 키 정의), dbt staging/마트 확장. DBTower 쪽은 필요 시 보존기간과의 정합(추출이 삭제보다 먼저)만 문서화 | lakehouse 쪽 DAG e2e(기존 quality gate 통과) |
| reverse ETL: 장기 베이스라인 | 미구현 | 방향 결정: DBTower가 DuckLake를 직접 읽는 대신(**duckdb JDBC 의존 추가를 피함**), lakehouse의 publish 단계가 메타DB에 `baseline_longterm`(instance_id, query_id, dow, hour, mean, stddev, observations, computed_at) 테이블을 **되쓰기**(reverse ETL 정석 — 웨어하우스 집계를 운영 DB로 sync). DBTower V14로 테이블만 정의, `BaselineService`가 자체 14일 베이스라인과 병합(장기 관측 수가 충분하면 가중 결합, 없으면 현행) — "보기"는 Metabase 유지, 되읽기는 **감지기가 소비하는 이 한 지점만** | 장기 테이블 주입 후 이상 판정이 계절성(예: 월요일 피크)을 오탐하지 않는 것 실측, 미존재 시 현행 동작 회귀 없음 |
| 용량 예측(디스크 포화 ETA) | 미구현 | 호스트 디스크는 DB가 아니라 노드 지표 소유: (a) 데모 compose에 node_exporter 추가, (b) 신규 `dbtower.prometheus.url` 설정 + `insight/internal/PrometheusClient`(RestClient, HTTP API `/api/v1/query`) — `predict_linear(node_filesystem_avail_bytes{...}[6h], 86400*N)`과 현재 여유 % 조회, (c) 인스턴스-노드 매핑은 등록 정보에 선택 필드(`nodeFilter` 라벨) V15, (d) `advisor/internal/DiskForecastAdvisor` — "N일 내 포화" CRITICAL/WARNING + 웹훅. **호스트 dedup 필수**: 디스크는 호스트 자원이라 같은 nodeFilter(=같은 서버)를 공유하는 인스턴스들에 경보는 호스트당 1건(Phase 4 호스트 그룹핑과 같은 축). RDS는 CloudWatch `FreeStorageSpace`가 소스이나 AWS SDK 추가는 후속(구조만 소스 추상화). **스케일 실행은 범위 밖** — RDS 스토리지 오토스케일·K8s PVC 확장 등 프로비저닝 계층(Phase C) 소유, DBTower는 신호까지(자동 페일오버 불가 논리와 동일). AWS도 오토스케일과 별개 경보를 카나리아로 권고([RDS Storage Autoscaling](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_PIOPS.Autoscaling.html)). Prometheus 미설정이면 Advisor 자체를 조용히 스킵(기능 게이트) | 데모 스택에서 대량 적재로 여유 감소 → 예측 ETA 산출·경보 실측. Prometheus 미설정 시 스킵 확인 |
