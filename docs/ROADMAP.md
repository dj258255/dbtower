...
## 심화 아크 3 — 테이블 상세 정보 KDMS 패리티 (착수 명세)

> **구현 담당: Opus. 착수 명세(미착수).** KDMS의 "테이블 상세 정보" 화면(스키마 정보=CREATE TABLE 전문,
> 기본 통계=엔진·행수·데이터/인덱스 크기·평균 행 길이·생성 시각, 인덱스 정보=컬럼·타입·카디널리티)을
> 5기종에서 재현한다. 심화 아크 2(문의 테이블 구조, 65절)의 "린 요약"을 풀 상세로 승격하는 다음 계단.
> KDMS 대비 현재 갭: 데이터/인덱스 크기·평균 행 길이 표시(원천은 tableStats에 있음), 인덱스
...
- 상세 패널: "관련 테이블 구조" 각 테이블을 KDMS처럼 **접이식 아코디언**으로 — 펼치면
...
- 라이브: 데모 MySQL(정수·문자열 컬럼+복합 인덱스 있는 테이블)에서 SHOW CREATE TABLE 원문·카디널리티
  ...
  스크린샷(KDMS 이미지와 대비). 존재하지 않는 테이블명·주입 시도 문자열 → 명확한 거부.
- 문서: VERIFICATION 절 추가, CHANGELOG, 블로그 14편 섹션(KDMS 패리티 대비표와 함께).
