...
### Phase 1 — 단일노드 프로덕션 하드닝 (완료 4/5, VERIFICATION 64절)
...
| 로그인 rate-limit | **완료(64절)** | `LoginAttemptGuard`+`LoginLockFilter` — 계정별 잠금, login.html 남은시간 표시. 라이브 11회째 차단 실측. 인메모리 노드별 한계는 Phase 3와 재검토(정직) |
| 로깅·커뮤니티 파일 | **완료(64절)** | 스프링 부트 네이티브 롤링 파일(janino 회피), CONTRIBUTING·CoC·이슈/PR 템플릿, README 시스템 요구사항 |
| actuator·API 토큰 안정화 | 미구현 | (a) `/actuator/prometheus` 선택 토큰(`dbtower.metrics.token` 설정 시 Bearer 검사, 미설정 시 현행+WARN). (b) `ApiTokenProvider.java:29` 매 기동 랜덤 재생성 → 최초 토큰을 메타DB 설정 테이블(V11)에 영속해 재시작 생존, ADMIN 재발급 | 재시작 후 토큰 불변, 메트릭 토큰 401/200 분기 |
