...
> 수준)·함정·검증 기준·산출물 체크리스트. 구현 담당(Opus)은 그 문서를 따른다.
...
### 테마 F — 스케일 제어 (Phase F 후보)
수집 병렬화(제한 워커+지터, 현재 직렬 루프) / 스케줄러 스레드 풀 분리(현재 폴러 7종이
스레드 1개 공유 — head-of-line blocking 실측) / 알림 글로벌 레이트리밋+폭주 묶음 발송 /
인스턴스별 수집 on-off·주기 오버라이드 / 헬스 스코어 주기 계산+캐시. 수천 대(노드 샤딩·
수집 에이전트 분리)는 설계 문서만 — 범위 밖.
