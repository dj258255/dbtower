...
| D9 | **심층 원인 진단 (왜 인덱스를 못 타나)** | 5기종 옵티마이저가 다 다름. 공통 열쇠=추정 행수 vs 실제 행수 괴리. [PG mis-estimate](https://pganalyze.com/docs/explain/insights/mis-estimate)·[Oracle E-Rows/A-Rows](https://jonathanlewis.wordpress.com/2016/05/05/e-rows-a-rows/)·[MSSQL 왜 인덱스 안 탐](https://www.sqlservercentral.com/articles/why-your-index-isnt-being-used-reading-execution-plans-to-find-the-real-culprit)·[Mongo executionStats](https://www.mongodb.com/docs/manual/tutorial/analyze-query-plan/) | explain()(B의 RuleBasedAnalyzer), latencyPercentiles, tableStats, D3 자연어 진단 | 지금 규칙은 **증상**(풀스캔·정렬)만 잡고 **추정만** 본다. 심층 진단으로: (1) **실제 실행 계획** — MySQL EXPLAIN ANALYZE, PG EXPLAIN(ANALYZE,BUFFERS), Oracle gather_plan_statistics+DISPLAY_CURSOR, MSSQL 실제 계획, Mongo explain(executionStats). `explainAnalyze(sql)` 새 능력(SELECT 전용, 대상 부하 주의 — 옵션/타임아웃). (2) **카디널리티 오추정 감지**: 추정 vs 실제 행수 10배+ 괴리를 최하위 노드에서 찾아 "여기가 원인" 지목(PG Rows Removed by Filter, Oracle E/A-Rows, MSSQL est/act, Mongo docsExamined÷nReturned). (3) **인덱스 무력화 근본원인 규칙**: 암시적 형변환(문자열↔숫자), 컬럼에 함수/표현식, 통계 노후 신호, 낮은 선택도, 복합인덱스 선두 누락 — ai-analysis-rules.md에 기종별 근거와 함께 추가. 정직: EXPLAIN ANALYZE는 실제 실행이라 위험 쿼리 주의(읽기 전용+타임아웃), 못 하는 기종/권한은 UNSUPPORTED | 실 5기종에서 형변환·풀스캔 쿼리에 대해 추정vs실제 괴리·근본원인 규칙이 실제로 지적하는지 라이브. 단위: 괴리 판정·규칙 매칭·UNSUPPORTED |
