...
| 6++ | 테이블 상세에 기본키·외래키(나가는 것·들어오는 것, ON DELETE·ON UPDATE)를 5기종 카탈로그에서 구조로, 워크벤치·관제가 같은 렌더러(참조 링크·외래키 열 인덱스 없음 표시·접힌 DDL), 그리드 열 너비·행 상세·열별 거르기·페이지 크기. 데모 외래키는 승인 티켓으로 올려 변경 계정의 REFERENCES 부족(MySQL·SQL Server 실행 거부)을 찾아 계정 구성 수정. 남은 과제: 실행 기록 구조 비교가 제약조건을 못 봄 | 151절 |
