...
| 백업 잔여(정석 완성도) | (1) **PG 물리 FULL**: pg_basebackup 타입 신설 — 현재 FULL은 pg_dump 논리 덤프라 WAL 재생 불가(PITR 문안에 한계 명시됨). 물리 FULL이 생기면 PG도 진짜 시점 복구 체인 완성. replication 권한 게이트. (2) **Mongo oplog ts 증분**: 현재는 매회 oplog 창 전체 덤프(capped라 자체 완결적) — 정석 증분은 마지막 수집 ts 이후만 query로 덤프 + 한 엔트리 겹침으로 연속성 검증(Pythian 방식). (3) PG 원샷 수집의 재활용 경합 창은 구조적 한계로 문서화됨 — 무결 보장은 pg_receivewal 스트리밍 별도 모듈로만 가능 | PG: pg_basebackup 실복원 e2e. Mongo: 겹침 엔트리 검증 |
