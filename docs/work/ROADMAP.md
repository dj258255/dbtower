...
| 단계 | 내용 | 검증 (VERIFICATION.md) |
|---|---|---|
| 0 | 자연어 진단 루프의 팀 범위 우회(서비스 토큰) 수정 — 대상 고정 가드 | 126절 |
| 0.5 | MySQL 모니터 권한 재실측 — "200 OK"가 아니라 권한 거부 메시지로 7개 누락 발견, 규약 검사로 고정 | 127절 |
| 1 | 워크벤치 조회 — 분리된 조회 계정, 읽기 전용 트랜잭션(쓰기 권한 계정으로 3기종 실증, Oracle 겹 추가), 마스킹, 기록 | 128절 |
| 2 | TOI식 워크시트 + AI 제안(실행 도구 없는 격리 LLM), 실행 결과로 채점한 정확도 lenient 12/12 | 129절 |
| 3 | 승인 티켓만 실행 — 조건부 실행권, 사본 행 수 = 영향 행 수일 때만 커밋, 드리프트 대조 뒤 되돌리기, 전후 비교 4종 | 130절 |
| 3+ | 취소·커밋 불명 정리·역변경 제안·테이블 상세, SQL Server 계열(Azure SQL Edge)·MongoDB 변경 실행, MCP 요청·조회 도구, 사본 암호화·요청자/승인자 분리·결과 카드 라이브, 부하를 건 인덱스 티켓 전후(pgbench 평균 49.634 -> 0.029 ms) | 131절 |
| 3++ | 한계 다시 파기: 실제 SQL Server 2022(Rosetta VM), MCP 요청자 신원 결함(자기 승인 200 -> 409), Oracle 앱 스키마, 기종별 행 지표 이름, 1ms 미만 정밀도·워크벤치 시각, 호스트 pgbench 재측정(46.378 -> 0.521 ms, 서버 안 0.0325 ms) | 132절 |
| 3+++ | 남은 제약: 이 Mac의 네이티브 x64 SQL Server 경로 조사(없음 — 공식 문서·이슈 근거), Rosetta 번역 비용 실측(PostgreSQL arm64 대 amd64 처리량 0.71·0.68배), `SELECT 1` 경로 분해(컨테이너 안 0.018 ms, 호스트 TCP 0.539 ms), Oracle 앱 스키마 인스턴스별 등록(V39) | 133절 |
| 3++++ | 네이티브 x64 러너(PR #2 워크플로)에서 SQL Server 2022 IT·부하 전후(러너 두 번 서버 53,052 -> 75.1·122,541 -> 173.0 µs로 흔들림, 계획·논리 읽기는 Rosetta VM과 동일), 정리 중 발견한 결함 수정 — 연결만 받고 말이 없는 대상 하나가 풀 생성 락으로 모든 폴러·삭제 API를 세우던 것(SQL Server·Oracle 25초 초과 -> 2초), main 병합 | 134절 |
| 4 | 한 플랫폼, 사람별 입구: 역할 5개(관제·요청자·승인자·운영자·관리자 — 승인자와 운영자는 서로의 일을 못 함, RoleHierarchy·V40), `/api/me` 능력 기반 화면 분기, 역할별 첫 화면(저장된 요청 우선), 넘김 셋(쿼리 상세 -> 워크벤치, 인덱스 제안 -> 변경 요청, 실행 기록 -> 대시보드 전후 비교), 사용자·역할 API·카드. 역할별 화면을 브라우저로 확인하며 결함 둘(워크벤치 안내 레이아웃, 리뷰 카드 UTC 시각) 수정 | 135·136절 |
| 4+ | 보여주기와 회귀 고정: 데모 compose PostgreSQL = postgres:16 + HypoPG, 대시보드 인덱스 제안이 정규화 텍스트($1·$2)에서 502이던 결함(GENERIC_PLAN 경로로 수정, Total Cost 12578.73 -> 8.47), 역할별 화면 E2E(Playwright, CI), 데모 GIF·한 장 요약·기술 Q&A | 137절 |
| 4++ | 화면 재질: Liquid Glass를 뜨는 계층(상단바·사이드바·세그먼트 탭·드롭다운·툴팁·모달·로그인)에만, 표·카드는 불투명. 보조 글자 대비 3.77 -> 4.97:1(유리 위 5.12:1), 투명도 줄이기·고대비·미지원 대비책 | 138절 |
| 4+++ | 레이아웃: 수집 안 된 차트의 빈 상자를 낮은 자리표시로(Metric 카드 1374 -> 638px, SVG에는 hidden 프로퍼티가 없어 속성 토글), 좁은 화면 넘침 수정(430px 대시보드 scrollWidth 784 -> 430, 한 열 그리드 minmax(0, 1fr)) | 139절 |
| 4++++ | 실시간 세션 관제: 대상별 채널 하나가 주기마다 한 번 조회해 SSE로 나누는 허브(구독자 0이면 조회 중단, 스코프는 구독 순간 확인, 실패·삭제도 프레임으로). 대상 pg_stat_statements 기준 30초·10명 150 -> 14회, 전원 떠난 뒤 0. 화면이 안 보이면 연결을 닫고 표 위 포인터에서는 갱신을 미룸, 실제 톰캣 경로 E2E | 140절 |
| 4+++++ | AI 응답 스트리밍: 워크벤치는 미완성 JSON에서 설명·SQL 앞부분만 흘리고 저장은 완성본(첫 설명 4.45·10.01·4.55초 vs 완성 10.85·19.96·12.10초), 진단은 단계 이벤트(첫 도구 6.38초, 결과 108.16초). API는 SDK 스트리밍+누적기, CLI는 stream-json. 작업 스레드에 SecurityContext 전달, 자리 없으면 503 | 141절 |
| 5 | 보여주기 갱신: 데모 GIF·README 화면을 새 디자인으로 재촬영(역할별 프록시에서 Playwright, 인덱스 티켓 #53 실행 뒤 역변경 #54로 정리). 촬영 중 찾은 결함 — 쿼리 상세 AI 분석·실행계획이 정규화 텍스트($1·$2)에서 502(137절 어드바이저와 같은 원인, GENERIC_PLAN 경로로 200). 로컬 검증 계정 정리, AGENTS.md 화면 규칙 | 142절 |
| 5+ | 쿼리 상세 AI 분석 스트리밍: 실행계획·규칙 지적을 AI 전에 먼저(0.02~0.05초, 한 번에 받기는 38~39초 동안 안 보임), 평문 조각은 80ms 묶음·끝에 반드시 flush, 한 번에 받는 REST와 같은 AiAnalysisRunner | 143절 |
| 5++ | 실시간 허브 노드 간 조율: 대상별 조회권(ShedLock)을 쥔 노드만 조회하고 UNLOGGED live_frame(V41)으로 프레임 교환, seq는 DB가 매김. 두 노드·노드마다 5명 30초 대상 조회 30 -> 15, 1차 결번(12·11장)을 재시도로 15·15장. 세션 쿼리 리터럴 가림(실시간·GET /sessions·AI 도구) | 144절 |
| 5+++ | 남은 AI 대기 두 화면 스트리밍: 변경 요청 소견(규칙 판정 0.01~0.05초 먼저, 한 번에 받기는 10~11초 동안 버튼만)·인시던트 리포트(재료 초안 0.04~0.12초 먼저, 한 번에 받기 15~18초). 흘리는 경로 인가 matcher 누락 방지, TextDeltaBatcher를 공개 모듈로, PRESENTATION 옛 화면 재촬영 | 146절 |
| 5++++ | 워크벤치 배치: 오른쪽 탭(채팅·스키마·변경 티켓)을 풀어 스키마는 인스턴스 아래, 티켓은 가운데 결과 탭, AI는 늘 보이는 칸. 테이블을 질문에 붙이는 클릭 4 -> 2회, 티켓 상세 398 -> 524px, 유리 판·대비 계산(미달 둘 수정) | 147절 |
| 5+++++ | 140~147절 재감사(보안·동시성·화면): AI를 기다리며 쥐던 플랫폼 DB 커넥션(흘려 받기 4건 동안 active 4 -> 0), 멈추거나 stderr가 찬 CLI 자식이 끝나지 않던 것(20초 넘김 -> 0.11·2.01초), 스트림 error 이벤트의 원문 오류, 이중 제출·늦게 온 응답, 죽은 딥링크 둘 | 148절 |
| 6 | 관제와 워크벤치를 한 페이지의 두 모드로: 상단 [관제 \| 워크벤치] 전환, 관제 모드 오른쪽에 늘 보이는 AI 칸(자연어 진단), 쿼리 넘김은 새 탭 대신 같은 페이지(탭 2 -> 1, 문서 로드 1 -> 0), 숨긴 모드의 실시간 연결 닫기, /workbench.html은 넘김 페이지 | 149절 |
| 6+ | 스키마 트리 깊이: 루트(데이터베이스·개수) -> 테이블/뷰 -> 열(기본키)·인덱스. 모델에 종류·기본키, 5기종 카탈로그 SQL, 뷰가 섞이던 PostgreSQL·SQL Server를 나눔(라이브 pg_stat_statements 2개 VIEW), MongoDB 뷰가 listIndexes로 스냅샷을 깨던 경로 차단 | 150절 |
| 6++ | 테이블 상세에 기본키·외래키(나가는 것·들어오는 것, ON DELETE·ON UPDATE)를 5기종 카탈로그에서 구조로, 워크벤치·관제가 같은 렌더러(참조 링크·외래키 열 인덱스 없음 표시·접힌 DDL), 그리드 열 너비·행 상세·열별 거르기·페이지 크기. 데모 외래키는 승인 티켓으로 올려 변경 계정의 REFERENCES 부족(MySQL·SQL Server 실행 거부)을 찾아 계정 구성 수정. 여기서 남긴 "실행 기록 구조 비교가 제약조건을 못 봄"은 153절에서 닫음 | 151절 |
| 6+++ | 변경 티켓 목록 범위 [열린 것 \| 전체]: 열린 티켓이 있으면 열린 것부터(처리할 티켓 15번째 -> 1번째), 끝난 티켓을 고르면 전체로 넓힘 | 152절 |
| 6++++ | 구조 스냅샷에 외래키: 실행 기록·인스턴스 구조 비교가 제약조건 변화(생김·사라짐·ON DELETE 바뀜)를 보인다. 151절에서 외래키만 추가한 실행이 "차이 없음"으로 남던 것을 닫음 | 153절 |
| 6+++++ | 리뷰 가능성도 품질: 소스에 들어간 원시 NUL 바이트가 Java·JS를 바이너리 파일로 보이게 하던 문제를 추적하고, 값은 유지한 채 `\u0000` 이스케이프 표기로 바꾼 뒤 규약 검사에 NUL 금지를 추가 | 154절 |
| 6++++++ | 구조 비교에 CHECK·트리거(정의·상태·수집 범위 표시). 승인 티켓으로 CHECK를 넣었다 뺀 실행 기록을 PostgreSQL·SQL Server에서 실측. 라이브가 SQL Server 권한 게이트의 결함(스키마 단위 VIEW DEFINITION을 못 봄)을 잡아냄 | 156절 |
| 6+++++++ | Oracle 외래키까지 승인 티켓으로 — 데모 테이블 하나에 최소 권한만 더해 티켓 4건(추가·삭제)으로 확인하고 스키마를 원래 구조로 되돌림. 변경 실행 5기종 라이브 완성 | 157절 |
| 6++++++++ | 쿼리 안티패턴 신호 5기종: 느림의 크기가 아니라 성질(인덱스 없이 훑음·디스크로 넘침·반환 행당 읽은 양)을 통계 뷰에서 읽는다. 축은 공통, 원천 지표 이름은 기종이 답하고 없는 축은 미확보로 표기 | 158절 |
| 6+++++++++ | 테마 A 재조사 — 플랜 플립은 이미 5기종이었고 원인은 환경·표본이었다. 티켓 상세를 카드로 갈라 읽히게 고치고, 가상 인덱스가 PostgreSQL(HypoPG) 전용임을 기종 선택 시점에 알림 | 159절 |
| 6++++++++++ | 테마 B·D 재조사 — 둘 다 이미 구현돼 있었다(데드락 축, p95 정직 등급). MongoDB가 계획을 뜰 수 없는 대체 식별자로 대상 DB에 조회를 보내던 것을 사전 차단 | 160절 |
...
| 단계 | 내용 | 검증 (VERIFICATION.md) |
|---|---|---|
| 161 | 5분 시연을 문서대로 완주 — 화면이 스스로 ADMIN API를 불러 403을 받던 것(수정 후 4xx 0), 관측 쿼리가 Top Query 상위를 차지하는 사실을 숨기지 않기로 | 161절 |
| 162 | 화면 지적 아홉 — 심층 진단 502는 자리표시자 입력 오류, `alert()` 5곳을 실패한 자리로, "CPU" 대신 5기종 자원 압박 축(한도 모르면 감점 없이 판정 보류), 버튼 유리 대비 재계산(5.71~15.80) | 162절 |
| 163 | 문서가 현재 구현을 가리키게 — Mermaid 원본 4장·SVG 4장, `ModularityTests` Documenter로 16개 모듈 문서 재생성, 역할별 수동 테스트. 테스트 890건 | 163절 |
| 164 | Apple Silicon Rosetta Colima에서 실제 SQL Server 2022(16.0.4275.2) 복구 — 변경·되돌리기·조회 권한 경계 5건 통과, 시간 경계 테스트 6,500ms 조정 | 164절 |
| 165 | 5기종 실제 값 불변식 QA 5건 — MySQL 서버 데몬 세션 오염 제거, 화면 NaN·음수 바이트 방어. 테스트 897건·E2E 9건 | 165절 |
| 166 | v1.3.0 릴리즈 게이트 통일 — 태그 push가 규약·E2E(skipped=0)·전체 테스트·멀티아치 GHCR·GitHub Release를 한 작업으로 | 166절 |
| 167 | 실제 값 재검사와 표 SQL 강조(흰 배경 대비 4.5:1 이상), 관제 대표 화면·7장면 GIF 재촬영, pg_stat_statements 자격증명 마스킹. 테스트 902건 | 167절 |
| 168 | Pi 로컬 에이전트 실행 정책 — 도구 호출 직전에 경로·명령·승인·검증을 재검사하고 `git push`·`kubectl`·운영 DB 클라이언트를 차단. 정책 테스트 19건 | 168절 |
| 169 | AI 운영 작업 — 진단 9종을 한 작업 모델로, 사실 수집·모델 호출·검증·감사는 플랫폼, 큐·검색·알림은 실행면. Slack 확인 응답 65.6ms, 결과 31.7초 | 169절 |
| 170 | 경보가 스스로 작업을 만드는 경로 실측 — 회귀->경보->작업->팀 채널 47.4~60.9초, 미매핑 팀은 Slack 미발송. 라이브에서 드러난 결함 9건 | 170절 |
| 171 | 남은 넷 수정 — 쿨다운 메타 DB 이관(`alert_cooldown`, V45)으로 재기동 중복 경보 0, 경보 작업 전체 상한, 같은 창의 비교 사실, 자리만큼만 읽는 파이프라인. 테스트 970건 | 171절 |
| 172 | 로그인 화면 재설계 — 단색 하늘 배경·카드 제거·그룹형 입력. 대비 계산이 미달 둘(4.49:1·2.35:1)을 걸렀고 390px 가로 넘침 0 | 172절 |
| 173 | 관제 AI 칸을 채팅으로 — 도구 호출·답 누적, 중지 버튼, 한글 조합 중 Enter 차단, 인스턴스별 sessionStorage 보관 | 173절 |
| 174 | 워크벤치 인스턴스 목록이 계정 복호화 실패 한 건으로 409로 비던 것 — 비밀번호 칸을 읽지 않는 생성자 조회로 분리 | 174절 |
| 175 | 화면 전체를 하늘 원톤으로 — 그라데이션 9곳 -> 0, 기본색 #4f5ef7 -> #0a6aa8, 입력 칸 모서리·테두리 통일(테두리 대비 1.25 -> 3.37:1) | 175절 |
| 176 | 관제 AI 대화 서버 저장(V46)과 슬로우 쿼리 값 가림 — 진단 맥락을 브라우저 `history` 대신 서버 기록에서. 테스트 1003건 | 176절 |
| 177 | 관제 AI 채팅 화면 — 서버 대화·대화 목록, 답 서식, 도구 이름, "쿼리 속 값은 ?로 가려서 보냅니다" 한 줄 | 177절 |
| 178 | 쿼리 상세 버튼 8개를 토글 5개+더보기로 — 누르면 바로 실행되던 심층 진단·문의를 확인 뒤로, SQL ID 축약·복사 | 178절 |
| 179 | 모니터링 카드 설명 22개를 사실 한 줄로, MCP 명령 복사에서 설명 분리, 역할 변경을 [적용]/[취소]로, 값 2개 이상일 때만 필터 노출 | 179절 |
| 180 | 워크벤치 — 조용히 멈추던 여섯 자리를 그 자리 사유로, 워크시트를 편집기 위 탭으로, 취소는 서버 왕복 없이. 테스트 1020건 | 180절 |
| 181 | 처음 들어온 사람의 화면과 좁은 화면 — 빈 자리에 한 문장, 기본 모양이 남은 입력·선택 정리, 390·768·1280px 가로 넘침 0 | 181절 |
| 182 | 대상 DB가 죽어도 관제 화면이 줄을 서지 않게 — 대상 조회 동시 2개, down 대상 조회 미전송. 대화 목록 10073ms -> 8ms, 동시 대상 조회 15 -> 2. 테스트 1041건·E2E 42건 | 182절 |
...
> 이후 재결정(이 결정의 기록은 그대로 둔다): 2026-07-18 판정·승인·기록까지(아크 B2), 2026-09-10 승인된 티켓만 플랫폼이 실행
> (워크벤치 3단계, VERIFICATION 130·131절). (a) MySQL 전용 문제는 실행 계층을 JDBC 표준 + 기종 훅 셋으로 만들어 5기종에서 풀었고,
> (b) "다른 DNA"는 실행의 안전을 SQL 해석이 아니라 진단과 같은 측정(사본 행 수·영향 행 수·드리프트 대조)에 걸어 좁혔다.
...
| CloudWatch 메트릭 소스 | **구현 완료(90절)** — HostDiskMetrics 소스 추상화(Prometheus | CloudWatch), MetricsSourceConfig 분기 1줄. CloudWatch는 FreeStorageSpace 최소제곱 회귀로 ETA(여유 %는 null — 총 용량 메트릭 부재를 정직 표기). "주입 지점 하나만" 약속 이행. 검증: 판정 단위(여유% null 축) + 실 SDK 컴파일 + LocalStack HTTP 왕복 도달 확인. 잔여: 완전 라이브 e2e는 SDK JSON ↔ LocalStack 3.8 query 프로토콜 격차로 실 AWS/RDS 필요(IT 게이트 보존) | 실 RDS/호환 에뮬레이터에서 IT 실행 — 코드·판정은 충족 |
...
| 항목 | 명세 | 검증 기준 |
|---|---|---|
| 1. 알림 → 진단 루프 | 레퍼런스의 "알럿 쓰레드 이모지 → AI 분석 댓글"을 셀프호스트 제약(봇 등록·이벤트 수신 인프라 없음)에 맞게 두 단계로. **1단계(지금)**: 회귀·이상·운영 경보 웹훅에 콘솔 진단 딥링크 포함 — 신규 설정 `dbtower.base-url`(빈 값이면 링크 생략), URL은 `{base}/?instance={id}&diagnose={질문 프리필}` 형태로 자연어 진단 입력을 미리 채움. 클릭 한 번으로 "이 알림 원인 분석해줘"가 실행되는 흐름. **2단계 — 구현 완료(88절)**: Discord Interactions 엔드포인트(/api/inbound/discord, 슬래시 커맨드 → DiagnosisService → 팔로업 댓글). 보안 3단계 그대로 — Ed25519 요청 서명(Java 표준 EdDSA, 401)·채널/유저 화이트리스트(기본 거부)·마스킹(70절 배선). 공개키 미설정=404 기능 게이트. 로컬 키쌍 서명 시뮬레이션으로 401/PONG/거부/DEFERRED+도구 3연쇄 실측 — 실 Discord 발사만 앱 등록(외부 계정) 필요로 잔여. Slack Events는 같은 구조의 변형이라 수요 시 | 1단계: 웹훅 실발사에서 딥링크 클릭 → 콘솔이 해당 인스턴스+질문 프리필로 열림. base-url 미설정 시 링크 없음 |
...
## 프로덕션·셀프호스트 준비도 감사 (2026-07-14 감사 기록 — P0·P1은 Phase 0·1에서 닫혔다)
...
| 백업 잔여(정석 완성도) | **전부 완료** — (1) PG 물리 FULL(75절): PHYSICAL 타입(pg_basebackup -Ft stdout) + 실제 시점 복원 e2e. (2) Oracle RMAN 경로(75절): rman 부재 실측으로 파일 수집 폴백. (3) **Mongo oplog ts 증분(83절)**: 파일명 ts 마커 기반 $gte 증분 — 겹침 엔트리 실복원 실증(전체 대비 1/28), oplog 순환은 명확한 실패. (4) **pg_receivewal 스트리밍(84절)**: WalStreamManager 상주 보살핌 + 복제 슬롯 — kill 후 30초 재시작·세그먼트 연속(공백 무손실) e2e, docker exec -i 좀비 CLI 함정 해소 | Mongo 겹침 엔트리 검증 — **충족(83절)**. pg_receivewal 프로세스 생존·재시작 e2e — **충족(84절)** |
...
| 메타DB 파티셔닝 | **완료(81절 — query_snapshot)** | V18 신테이블→복사→스왑(단일 트랜잭션, PG 16 identity 미지원이라 시퀀스 DEFAULT, PK에 파티션 키 포함). 보존 잡이 파티션 수명주기 관리자: 이번 달·다음 달 선생성(멱등) + 월 전체 경과 파티션 DROP + 걸친 파티션 내부만 DELETE(프루닝) + 비파티션(H2·전환 전) DELETE 폴백. DEFAULT 파티션 안전망. pg_partman 의존 없음. health_sample도 V19로 완료(85절 — PartitionLifecycle 공용화, 보존 잡은 이미 있었음) | **충족(81절)**: DELETE 200만 행 1,879.9ms → DROP 12.8ms(147배·블로트 0), EXPLAIN 프루닝(7월 조회가 6월 파티션 무접촉), 스왑 부산물로 블로트 404MB→55MB |
| 호스트 그룹핑(서버 공유 인지) | **완료(79절, 경보·스윕·UI)** | 구현: `DatabaseInstance.serverKey()`(host 소문자:port 계산 키 — 엔티티 추가·DNS 해석 없음) 파생 그룹. **dedup은 탐침·경보에만, 위험 귀속(온디맨드 점검·헬스 스코어)에는 안 한다** — 같은 서버의 두 DB가 모두 위험한 건 사실. (1) OpsAlertDetector: 서버 전역 4종(유휴 트랜잭션·복제 지연/슬롯·데드락)은 그룹 대표(id 최소)만 탐침, 공유 서버면 경보에 "(서버 x 공유 — ... 전체에 해당)" 명시, 인스턴스 스코프(수집 정지·백업 신선도)는 각자 유지. (2) AdvisorSweepJob: 호스트 스코프 Advisor(`Advisor.hostScoped()`, 디스크 예측)는 호스트+nodeFilter 그룹당 1회, 나머지는 SHARED 상태로 정직 표기. (3) 콘솔 카드에 "서버 공유 ×N" 배지. **잔여(조사 완료 — 87절 회차 실사)**: 수집 경로의 서버 전역 중복은 wait event·parameters가 아니라(둘 다 주기 영속 없음) **query_snapshot 자체**에 있다 — 실사 결과 MySQL digest(SCHEMA_NAME 무필터)·MSSQL DMV(dm_exec_query_stats 무필터)는 서버 전역이라 같은 서버의 두 DB가 같은 통계를 이중 저장한다(PG만 dbid 필터로 DB 스코프). 그런데 해법 둘 다 트레이드오프가 크다: (a) 스키마/dbid 필터링은 MySQL의 NULL 스키마·MSSQL의 ad hoc(dbid NULL) 문장을 유실시키고(가시성 손해), (b) 서버 그룹 대표만 수집하면 비대표 인스턴스의 Top Query·시점 비교 화면 계약이 깨진다(화면의 서버 스코프 재설계가 선행 조건). 중복은 오보가 아니라 저장 낭비뿐이므로 수천 대 실수요 전까지 보류 — 이 문단이 그 판단의 기록이다 | 같은 서버 2 DB에 실 유휴 트랜잭션 → 경보 1건+공유 명시, 인스턴스 스코프는 각자 — **충족(79절)**. 스윕 로그 호스트공유생략=5 실측 |
...
### Phase 5 — 분석 루프 (PostgreSQL 장기 집계 전환)

| 현재 항목 | 상태 | 다음 완료 조건 |
|---|---|---|
| 외부 lakehouse·Metabase 연동 | **제거** | V50 적용 뒤 `baseline_longterm` 수신 테이블과 MCP 도구가 없는 것 |
| 원본 스냅샷 | **유지** | 짧은 보존 기간과 파티션 수명주기를 계속 적용 |
| PostgreSQL 장기 집계 | **계획** | 시간·일 집계의 보존 기간, 집계 범위, 원본 삭제와의 정합성을 먼저 명세화 |

아래 표는 2026-07 당시 lakehouse 설계·검증의 보존 기록이다. 현재 제품의 계획이나 지원 범위가 아니다.

| 항목 | 상태 | 구현 명세 (Opus) | 검증 기준 |
|---|---|---|---|
| forward: offload 확대 | 부분 — lakehouse 쪽 D3·D4(레지스트리·게이트 프로필, backup_run·plan_snapshot 추출) 완료(그쪽 14단계), **선결 2(D2) 완료(100-3절)**: plan 보존에 48h 시간 하한 병행. **D1 완료(101절)**: V25 + WaitEventSnapshotJob(5분 수집·7일 보존) — lakehouse 편입 완료(그쪽 14-5절). forward 선결 전부 해소 | 실분석(2026-07-16) 결과 현재 추출은 `query_snapshot` **하나뿐**이고 "코드 변경 0" 전제는 backup_run·plan_snapshot에만 참: **(선결 1) wait event는 메타DB에 영속 테이블이 없다** — DBTower가 `wait_event_snapshot(instance_id, captured_at, event_name, category, value)` 주기 영속을 먼저 낳아야 함(lakehouse ROADMAP 13단계 size_snapshot과 동일 패턴). **(선결 2) plan_snapshot 보존이 카운트 기반(쿼리당 최신 20, 1시간 스윕)** — "어제 하루창 추출"과 어긋나 하루가 닫히기 전 행이 지워질 수 있음 → 시간 기반 보존 병행 또는 추출 주기 단축의 정합 문서화. **(선결 3) backup_run은 사후 변이 테이블**(verify/remote가 나중에 UPDATE) — "닫힌 dt 불변" 전제 깨짐, D+1 스냅샷임을 계약 명시 필요 + 워터마크 컬럼이 started_at. lakehouse 쪽 작업은 SOURCE_TABLE 단수 상수의 테이블 스펙 레지스트리화, 게이트 4축 테이블별 프로필(completeness·freshness를 그대로 재사용하면 backup_run의 정상 상태가 fail-closed 오탐), CONTRACT §1 개정, GRANT 목록 추가 등 10개 파일 동심원 — **착수 명세를 lakehouse ROADMAP 14단계로 이관(2026-07-17, D1~D4·선결 포함)** | lakehouse 쪽 DAG e2e(테이블별 게이트 프로필 통과) + plan_snapshot 보존 정합 실측 |
| 자연어 서빙(장기 마트 MCP) | **완료(103절)** — `lakehouse_query`(SELECT 가드)·`lakehouse_card_create`(전용 컬렉션) 2종, Metabase API 경유(DuckDB 의존 0). 카드 76 실물 생성. lakehouse ROADMAP 15단계의 DBTower 몫 |
...
> 2026-09-10 재결정: 아래 R6의 "플랫폼이 실행하지 않는다"를 "승인된 티켓만 변경 계정으로 실행한다"로 바꿨다(사용자 결정, 워크벤치
> 3단계). 사람이 DB 도구로 따로 실행하면 승인된 SQL과 실행된 SQL이 같다는 증거, 되돌릴 사본, 전후 비교가 남지 않아서다.
> 대형 테이블 DDL은 R6의 gh-ost 안내를 그대로 둔다. 상태 전이는 `ChangeTicketGate`의 조건부 UPDATE로만(VERIFICATION 130·131절).
...
### 아크 B3 — 인덱스 사용 통계 주기 영속 (장기 관측의 원료, V25·V26 패턴)

"이 인덱스 지워도 되나"는 재시작-누적 카운터의 순간 관측으론 답할 수 없다(지난주 재시작이면
전부 0). 분기 단위 "스캔 0" 판정은 장기 집계 설계의 몫이고, DBTower는 원료(주기 영속)를 낳는다.
...
- **승인 없는 변경 실행** — 2026-09-10 재결정으로 **승인된 티켓만** 플랫폼이 실행한다(워크벤치 3단계,
  VERIFICATION 130·131절). 이전 결정("승인된 변경도 플랫폼은 실행하지 않는다")을 뒤집은 근거: 사람이 DB 도구로 따로 실행하면
  "승인된 SQL = 실행된 SQL"의 증거도, 되돌릴 사본도, 전후 비교도 남지 않는다. 대신 조건을 코드로 건다 — 실행권은 조건부 UPDATE로
  한 요청만, 사본 행 수와 영향 행 수가 같을 때만 커밋, 되돌리기는 실행 직후 사본과 현재 행이 같을 때만. 여전히 안 하는 것:
  승인 없는 실행, 에이전트(AI·MCP)의 실행 도구, 대형 테이블 DDL의 직접 실행(gh-ost 경로 유지).
  2026-09-18에 **대량 일괄 변경**을 더했다(#101~#105, VERIFICATION 196·197절) — 수십만 행은 행 사본 방식의
  대가가 행 수에 비례해(10,000행 락 보유 67~75ms) 쓸 수 없었고, 그래서 플랫폼 밖에서 손으로 돌던 일이다.
  안전의 근거를 바꿔 들여왔다: 되돌리기는 사본이 아니라 **복원 검증된 최근 백업**을 실행 전 조건으로 걸고,
  구간마다 따로 커밋하며, 영향 행 수가 목표 배치 크기를 넘으면 커밋하지 않는다. 취소·실패는 이미 커밋한 배치를
  되돌리지 않는다 — 부분 적용이 정상 상태다.
  2026-09-18에 범위를 넓혔다(#126~#129, v1.6.0) — 복합 기본 키를 사전순 행 값 비교로 자르고, Oracle·SQL Server·MongoDB까지
  5기종 전부에서 돈다. 넓히면서 **담지 않는다고 판정한 것**도 명세에 적었다. 여러 테이블에 걸친 변경은 대상이 1:N 조인의
  N 쪽일 때 배치 하나가 한 스냅샷이라는 성질이 깨져 담지 않는다. MongoDB는 `_id` 타입이 섞이면 `$gt`가 타입 경계를 넘지 못해
  문서가 오류 없이 빠지므로 실행 전에 거부한다. 여전히 안 하는 것: 기본 키 없는 테이블, 여러 테이블에 걸친 변경,
  `_id` 타입이 섞인 컬렉션, 샤딩된 컬렉션(판정에 필요한 실측을 아직 하지 않았다).
...
- **자동 페일오버** — HA 토폴로지의 소유자(Operator/managed 서비스)가 할 일. 관제 도구가 페일오버를
  ...
  **관측은 구현됨**(VERIFICATION 123.17): 역할 변경(승격/강등)·split-brain(한 클러스터에 쓰기 가능
  노드 둘 이상)을 감지·경보한다. 실제 PG 페일오버로 라이브 검증 — 실행(펜싱·정족수)은 그대로 소유자 몫.
- **메타 DB를 시계열 저장소로 쪼개는 것**(v1.6.2 판정) — 리소스 메트릭(CPU·커넥션·디스크)은 이미
  Prometheus + exporter 셋이 맡아 경계가 서 있다. `query_snapshot`을 그쪽으로 옮기지 않는 이유는
  `query_id`가 인스턴스마다 수천 종이고 계속 바뀌어 라벨 카디널리티가 터지며, `query_text` 4000자를
  담을 자리가 없어서다 — "어제와 비교해 느려진 쿼리의 SQL을 보여 줘"가 제품의 핵심 기능이다.
  셀프호스트 한 대 규모에서 저장소를 하나 더 두면 설치하는 사람이 백업·업그레이드·장애 지점을 둘
  관리하게 된다. 장기 보관도 메타 PostgreSQL의 집계 테이블로 유지한다(VERIFICATION 201절).
- **감사 기록에 보존 기한을 두는 것**(v1.6.2 판정) — `audit_event`·`workbench_query_log`는 영구 보존한다.
  "그때 무슨 일이 있었나"를 나중에 묻기 위한 기록이라 기한을 두면 그 물음에 답할 수 없다. 대신 남기는
  것을 좁혀 양을 잡는다(`AuditPolicy`가 POST·PUT·DELETE만). 규정상 기한이 필요한 설치는 직접 지운다
  (`docs/operate/operations.md` 8절에 SQL을 적었다).
