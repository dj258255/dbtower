...
**표시 순서 원칙**: 근본원인이 매칭되면 원인을 카디널리티 괴리보다 먼저 제시한다 — 괴리는 원인의
증상이라, 증상이 먼저 보이면 사용자가 "통계 갱신(ANALYZE)"이라는 엉뚱한 처방으로 빠진다. 괴리가
헤드라인이 되는 건 원인을 못 찾았을 때뿐이다. loops 총량 환산 안내도 loops>1인 노드를 실제로
봤을 때만 붙인다(항상 붙이면 노이즈).

| 원인 | 왜 인덱스를 못 타나 | 감지 신호 | 특히 심한 기종 |
|---|---|---|---|
| 암시적 형변환 | 비교 규칙상 문자열 쪽이 숫자로 캐스팅된다(CAST(col AS DOUBLE) = N 꼴) — 인덱스 컬럼에 함수를 씌운 것과 같아 seek이 scan이 된다. 서로 다른 문자열('1', ' 1', '1a')이 같은 숫자로 매핑되므로 문자열 순서 B-Tree로는 숫자 등치를 탐색할 수도 없다. **성능보다 정합성이 먼저다**: '0N'·' N'·'Nabc'까지 전부 매칭돼 조회면 오답, UPDATE/DELETE면 데이터 사고 | 컬럼 타입 vs 비교값 타입 불일치. MSSQL은 플랜에 CONVERT_IMPLICIT 경고 | MySQL·MSSQL (VARCHAR 컬럼에 INT 파라미터가 단골) |
