...
마지막으로 정석 완성도 아크(VERIFICATION 83~95절)로 백업·자격증명·채널을 현업 표준까지 밀었다 —
**백업의 정석 4종**: Mongo oplog ts 증분($gte 겹침 한 건이 체인 무결의 증거, 산출물 1/28),
pg_receivewal 상시 스트리밍(복제 슬롯 — kill 후 재시작 사이 유실 0 실측), MySQL 물리 백업
XtraBackup(검증은 파일 존재가 아니라 실제 --prepare 실행), 산출물 AES-256-GCM 암호화(GCM 태그로
변조가 조용한 오염 대신 명확한 실패가 된다). **자격증명 두 축**: Vault 동적 계정(발급→선제 갱신→
소멸 전체 수명주기 실측 — 유출 창이 TTL로), MCP OAuth 2.1(브라우저 로그인으로 토큰 자동 발급,
커밋 자동 리뷰가 잡은 redirect_uri userinfo 우회까지 수정). **채널 완성**: 감지 알림 리치 embed +
Discord 봇 양방향 — 슬래시 커맨드(Ed25519 실서명 검증, Discord PING 통과)와 이모지 반응 봇
(특권 인텐트 0 — 발사 시점 message_id 매핑을 메타 DB에 영속해 재시작 생존, 알림에 돋보기를 달면
AI가 도구를 연쇄 호출해 embed 카드로 답글). 진단이 하트비트를 굶기던 스레드 공유 버그 같은
운영 결함도 실측으로 잡아 기록했다.

남은 것은 외부 의존 잔여뿐이다(정직 명시): CloudWatch 라이브 e2e(실 AWS/RDS 필요 — 코드·단위·
IT 게이트는 완비), lakehouse 연동(별도 저장소 — 착수 명세는 그쪽 ROADMAP 14단계로 이관).
