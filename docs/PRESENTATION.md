...
제공한다 (VERIFICATION 17절 · 71절 · 131절 · 132절). compare, explain, wait_events, sessions, partitions,
...
에이전트 도구로 열지 않았다. HTTP 전송은 도구 호출을 서비스 토큰이 아니라 그 요청을 인증한 호출자의 토큰으로 REST에
위임한다. 서비스 토큰으로 위임하던 때는 MCP로 올린 요청의 요청자가 사람이 아니라 `api-token`으로 남아, 같은 관리자가 자기
요청을 승인해도 요청자·승인자 분리가 막지 못했다(132절, 수정 전 200 -> 수정 뒤 409). 등록은 한 줄이다.
...
![MCP 클라이언트가 authorize를 열면 뜨는 DBTower 로그인 창 — authorize는 같은 로그인 화면으로 보낸다(142절 새 디자인)](images/webui/105-glass-login.jpg)

## 9. AI 운영 작업 — 화면 앞에 사람이 없어도 도는 진단

기존 AI(회귀 1차 분석·워크벤치 보조·자연어 진단·실행계획 분석)는 전부 사람이 화면 앞에 있어야 돌았다. 요청과
결과 사이가 수십 초라 Slack 3초 규칙도 못 지키고, "이 소견이 어떤 사실을 보고 한 말인가"를 되짚을 방법도,
모델이 지어낸 수치를 거를 방법도 없었다. 그래서 진단 종류 9종(쿼리 진단·회귀 원인·백업 위험·SLO 위험·Advisor
요약·비용 검토·장애 초기 진단·DB팀 문의·정기 리포트)을 **하나의 작업 모델**로 묶고, Slack 한 문장·웹 콘솔·경보·
DB팀 문의 어디서 시작하든 같은 권한·사실·검증·감사를 거치게 했다 (169절).

경계는 "틀렸을 때 손해가 큰 일"에 그었다.

| 하는 일 | 어디서 | 왜 |
|---|---|---|
| 권한·팀 범위, 사실 수집, 모델 호출, 결과 검증, 감사 | DBTower | 실행면이 사실을 만들어 오면 지어낸 값을 가려낼 기준이 사라진다 |
| 큐·재시도·참고 자료 검색·알림 | 실행면(Python) | 플랫폼은 Redis를 모른다 — 큐 장애도 플랫폼을 흔들지 않는다 |

승인 대기 상태는 두지 않았다. 변경 승인은 review 모듈이 단일 권위라, AI 작업에 승인 상태를 따로 두면 권위가 둘이
된다. 변경이 필요하면 결과에 `approvalRequired`만 남기고 기존 변경 요청 티켓으로 보낸다.

모델이 지어낸 수치와 인용은 플랫폼이 모은 사실과 글자로 대조하고, 어긋나면 거부하지 않고 "검증되지 않음" 목록으로
남겨 소견보다 먼저 보여준다. 이 검사는 실측에서 두 번 틀렸고 둘 다 고쳤다 — 분석 구간 시각
`2026-09-15T16:49:11Z`를 숫자 다섯 개로 쪼개 정직한 답변에 경고 여섯 줄을 붙였고(시각을 한 값으로 대조),
단위가 붙은 `350ms`를 아예 못 봐 지어낸 값이 통과했다(단위를 허용).

실측(라이브 2차 관통): Slack 확인 응답 65.6ms, 접수 답글 +0.2초, 결과 31.7초(3차 42.1초). 단계별로 접수->선점
0.98초, 사실 수집 0.22초, 참고 자료 검색+모델+검증 30.3초. 남은 한계는 참고 자료 검색 품질이다(상위 3개 중 관련 절 11/27).

## 10. Lessons Learned — 밟은 지뢰와 해결

레퍼런스가 공유한 함정을 직접 재현해 확인했고, 그 과정에서 우리만의 지뢰도 몇 개 더 밟았다.

### 10.1 digest 저장 길이 (MySQL)
...
### 10.2 digest 저장 건수 포화와 PS 사각 (MySQL)
...
### 10.3 pg_stat_statements는 클러스터 전역이다
...
### 10.5 타임존 스큐 — 그래프가 9시간 미래를 조회하고 있었다
...
- 거버넌스 SQL 워크벤치(128~132절): DBeaver처럼 대상 DB를 직접 조회하되 정책은 플랫폼이 강제한다 — 분리된 조회 계정,
  ...
  커밋하고, 되돌리기는 실행 직후 사본과 지금 행이 같을 때만 쓴다. 5기종(SQL Server는 Azure SQL Edge와 Rosetta VM의 실제
  SQL Server 2022 RTM-CU26, MongoDB는 복제셋 트랜잭션)을 실DB로 검증했고, 부하를 건 인덱스 티켓 전후를 pgbench로 쟀다(131·132절).
  Apple Silicon에서 SQL Server 네이티브 경로가 없다는 것은 공식 문서·이슈로 확정하고, 같은 Rosetta VM에서 PostgreSQL arm64 대 amd64로
  번역 비용(처리량 약 0.7배)을, `SELECT 1`로 측정 경로 비용(호스트 TCP 0.539 ms)을 쟀다(133절). 네이티브 x64는 GitHub Actions 러너에서
  같은 IT·같은 부하 측정기로 쟀고(러너 두 번 서버 실행 53,052 -> 75.1 µs, 122,541 -> 173.0 µs — 실행마다 흔들려 환경 간 µs 비교는
  쓰지 않고 계획·논리 읽기가 같다는 것만 쓴다), 정리 중 연결만 받고 말이 없는 대상 하나가 풀 생성 락으로 모든 폴러와
  삭제 API를 세우던 결함을 재현 테스트로 잡아 고쳤다(25초 초과 -> 2초, 134절).
  화면은 참고 도구의 탭 모양을 옮겼다가 쓰는 흐름으로 다시 나눴다(147절): 스키마는 인스턴스 아래, 변경 티켓은 가운데 결과 탭,
  AI는 늘 보이는 오른쪽 칸. 테이블을 질문에 붙이는 클릭 4 -> 2회, 티켓 상세 398 -> 524px을 같은 스크립트로 쟀다.
- 한 플랫폼, 사람별 입구(135절): 관제와 워크벤치를 두 제품으로 나누지 않고 쓰는 사람을 나눴다. 관제(VIEWER)·요청자·승인자·운영자·관리자
  다섯 역할에서 승인자와 운영자는 서로를 포함하지 않아 한 사람이 승인하고 실행까지 하는 흐름이 역할에서부터 없다. 요청자는 로그인하면
  워크벤치로, 나머지는 대시보드로 가고, 쿼리 상세·인덱스 제안·실행 기록이 두 화면 사이를 한 번에 넘긴다.
