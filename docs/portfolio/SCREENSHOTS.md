...
UX 2차(#65 -> #55~#64)로 바뀐 화면입니다. 위와 같은 개발 앱에 붙은 실제 PostgreSQL 16(live-postgres-team-b)에서
2026-09-18 오전에 찍었습니다. Top Query 목록에 `bulk_scale` 문장이 보이는 것은 같은 날 돌린 100만 행 실측(198절)이
통계에 남긴 것입니다.

![관제 첫 화면 — 확인이 필요한 DB 5대, 감점 사유 칩, 오른쪽 AI 대화 칸의 방식 셋(#55·#56·#57)](../images/webui/200-console-attention.jpg)

카드 한 장이 "지금 손볼 DB가 몇 대이고 무엇 때문인가"에 답합니다. 사유 칩(가용성 −45, SLO / 버짓 −25, 백업 신선도 −20)을
누르면 그 원인을 보는 화면으로 바로 갑니다 — 예전에는 인스턴스 선택부터 행 클릭까지 다섯 단계였습니다.

![쿼리 상세 — PostgreSQL 실행계획을 노드 트리로, 자기 몫 비용이 가장 큰 노드를 강조(#83)](../images/webui/201-querydetail-plan-tree.jpg)

수백 줄 JSON 대신 노드 종류·대상·조건·비용·추정 행을 한 줄씩 보이고, 자기 몫 비용이 가장 큰 노드에 표시를 답니다.
원문 JSON은 접어 둡니다.

![AI 대화 칸 — 입구 하나에서 지금 답하기·작업으로 맡기기·실제 실행 진단을 고른다(#57)](../images/webui/202-console-ai-chat.jpg)

예전에는 관제 채팅·쿼리 상세 AI 분석·심층 진단·작업으로 맡기기가 각자 입구였습니다. 지금은 대화 칸 하나이고,
입력 위에서 방식을 고릅니다. 쿼리 상세의 "AI에게 묻기"는 그 쿼리를 이 칸에 칩으로 붙입니다.

## 대량 일괄 변경 (197절)

승인된 티켓을 기본 키 범위 배치로 쪼개 돌리는 화면입니다. **실제 대상 DB가 아니라 브라우저 테스트가 채운 응답으로
찍었습니다**(`BulkChangeProgressE2ETest`) — 진행 중인 실행을 라이브에서 붙잡아 찍으려면 100만 행 변경을 돌리는 동안
촬영해야 해서, 화면 배치와 표기를 보이는 목적으로 테스트 캡처를 씁니다. 위 갤러리의 다른 이미지는 모두 라이브 촬영입니다.

![워크벤치 대량 일괄 변경 — 진행 패널, 일시정지·취소, 배치별 키 구간과 복제 지연 표기](../images/webui/199-workbench-bulk-change.png)

표의 "복제 지연" 열이 이 화면의 요점입니다. 실측한 배치는 `0.4초`, 못 읽은 배치는 `못 읽음`으로 적습니다 —
"지연 0"과 "못 쟀다"를 같은 값으로 보이면 나중에 "왜 안 멈췄나"를 되짚을 수 없습니다.
...
현재 화면으로 소개하지 않고 [VERIFICATION 162절](../verify/VERIFICATION.md)에만 보존합니다. 현재 구현은
