...
링크를 `/?aiop=<작업id>` 딥링크로 바꿨다. 딥링크로 들어가면 Monitoring 탭과 진단 그룹을 열고 해당 작업을 펼친다.
...
- **경보 자동 트리거**: 이벤트 발행과 접수는 단위 테스트로 확인했고, 기본은 꺼져 있다. 실제 경보에서 작업이 생기는 라이브 관통은 아직이다.
- **정기 리포트**: 유형과 사실 수집은 있으나 주기 실행 스케줄러는 없다.
