...
- **4·5·6·7은 고치지 않고 남긴다.** 넷 다 설계 판단이 필요하다 — 쿨다운을 어디에 영속할지, 기동 직후 경보를 어떻게
  묶을지, 경보가 본 구간을 사실에 어떻게 실을지, 실행기의 읽기 루프를 어떤 모양으로 바꿀지. 관측과 원인까지만 적는다.
- 같은 시간대 함정이 `MonthlyReportJob`(`0 0 9 1 * *`, zone 없음)에도 있다. aiops 밖이라 이번에 건드리지 않았다.
