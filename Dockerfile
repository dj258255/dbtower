...
# AI 1차 분석의 판단 기준(dbtower.ai.rules-path=docs/design/ai-analysis-rules.md, 작업 디렉터리 /app 기준).
# 이미지에 포함하지 않으면 AiAnalyzer가 빈 프롬프트로 판정해 "일관 판정" 가치가 사라진다.
COPY docs/design/ai-analysis-rules.md /app/docs/design/ai-analysis-rules.md
