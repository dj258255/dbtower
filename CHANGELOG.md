...
- **관측이 대상을 얼마나 더 느리게 만드는지 쟀다**(E4 확장). 같은 40초 창에 앱 부하를 걸고 정책별로 비교해, 매초 겹쳐 보내는 방식은 앱이 같은 시간에 처리한 조회가 7만 건에서 5만 건으로 29% 줄고 p99이 1ms에서 4ms로 늘었다. 끝난 뒤 다음 수집(제품)은 관측을 하지 않은 기준선과 구분되지 않았다. `docs/experiments/ash-backpressure.md`
