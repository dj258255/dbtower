...
- **AI로 나가는 값의 가림을 기본값으로 켰다**(#131 재판단). `dbtower.masking.mask-ai-prompt`·
  `mask-ai-plan` 기본값을 `false`에서 `true`로 바꾸고, 수준은 `STRUCTURE`(값은 지우고 `%` 자리·자릿수·날짜
  거리만 남김)를 유지한다. #131은 "가림을 켤 것인가"와 "어느 수준으로 켤 것인가"를 판정 일치 80% 기준 하나로
  묶어 둘 다 보류했는데, 앞의 질문은 실험이 아니라 개인정보 판단이다. 실측에서 외부로 나가는 원문 민감값은
  원문 65건 대 가림 0건이고, 대가는 이 세션 판정 기준 42건 중 3건이다(NONE 38/42 → STRUCTURE 35/42).
  손실 폭의 절대값은 판정자마다 다르지만 순서는 셋이 일치한다. 값이 밖으로 나가지 않는 사내 설치 모델에만
  `NONE`을 쓴다. 근거와 수치는 `docs/experiments/ai-masking-levels.md`

### 잰 것

- **관측이 대상을 얼마나 더 느리게 만드는지 쟀다**(E4 확장). 같은 40초 창에 앱 부하를 걸고 정책별로 비교해, 관측은 대상 CPU를 1~2%p 올렸고 디스크 읽기는 세 조건 모두 0이었다(관측 조회가 캐시에 적중). 앱 조회 지연은 조건 간 차이가 없었다. **같은 조건을 3회 반복하니 실행 간 변동(관측 없음 5.6만~7.6만, PARALLEL 5.3만~7.0만)이 조건 간 차이(중앙값 기준 5천 이내)보다 컸다** — 1회 측정으로는 관측의 한계 비용을 가를 수 없었고, 관측 때문에 대상이 느려진다는 증거는 확정하지 못했다. `docs/experiments/ash-backpressure.md`
- **샘플 간격을 늘리면 사건을 놓친다**(E4 확장). 같은 조건(지연 0ms)에서 계획 간격만 1·2·4초로 바꿨다. 1초는 세 정책 모두 10/10을 잡았고, 2초는 DELAY 0/10 · QUEUE 9/10 · PARALLEL 0/10, 4초는 전부 0/10이었다. 사건이 4초마다 1.5초 지속되므로 간격이 사건 주기에 가까워지면 겹치지 않고 지나간다. 그래서 기본값 1초를 유지하고 게이트도 1초 조건에만 건다. `docs/experiments/ash-backpressure.md`
- **배치 사이 쉼이 복제 지연을 줄인다는 기대는 기각됐다**(E6b). primary/streaming replica를 세우고 100만 행 배치의 쉼만 0·100·300ms로 바꿨다. 작성자 없이 쉼 자체의 효과를 보면 lag 최대가 9 → 77 → 165ms로 오히려 늘고, 작성자 둘에서는 125 → 862ms로 늘었다. 총 소요는 6.4초 → 316.9초로 50배, writer 최대 대기는 120 → 620ms가 됐다. 1,000행 배치는 동시 작성자 지연 실측에서 온 값이지만 **100ms는 근거가 확인되지 않은 정책값**이다. `docs/experiments/replication-pause.md`
그 사이 보안·데이터 손상 결함만 패치 버전으로 낸다.

### 고침

- **대량 일괄 변경이 배치 경계로 쓰는 기본 키 열을 바꾸는 문장을 실행 전에 거부한다**. 경계를 키로 잡고
  그 키가 커지는 방향으로만 나아가므로, 키가 바뀌면 이미 지나간 구간으로 이동한 행이 조용히 빠지거나
  다시 처리된다. 오류가 나지 않아 누락을 알리지 않던 자리다(`ChangeStatementParser.updateSetColumns`,
  `BulkChangePreflight.requireKeysNotModified`, VERIFICATION 202절)
