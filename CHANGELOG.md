...
- **관측이 대상을 얼마나 더 느리게 만드는지 쟀다**(E4 확장). 같은 40초 창에 앱 부하를 걸고 정책별로 비교해, 관측은 대상 CPU를 1~2%p 올렸고 디스크 읽기는 세 조건 모두 0이었다(관측 조회가 캐시에 적중). 앱 조회 지연은 조건 간 차이가 없었고 처리량은 실행마다 7만~7.7만으로 흔들려, 관측 때문에 대상이 느려진다는 증거는 확정하지 못했다. `docs/experiments/ash-backpressure.md`
