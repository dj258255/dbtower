...
- **적용된 마이그레이션 파일은 고치지 않는다 — 주석 한 글자도.** Flyway 가 파일 내용으로 체크섬을 내고
  `validate-on-migrate` 기본값이 켜져 있어, 바꾸면 이미 적용한 설치가 **기동하지 못한다**.
  파일 안의 문서 경로가 낡아도 그대로 둔다 — 낡은 경로는 읽는 사람의 불편이고 체크섬 불일치는 장애다.
  고칠 것이 있으면 새 버전 파일을 더한다
- **실제 PostgreSQL 에 돌려 보고 커밋한다.** H2 테스트는 JPA 엔티티로 스키마를 만들어 주므로
  마이그레이션이 틀려도 통과한다(V48 의 `AUTO_INCREMENT`, V49 의 컬럼 정의가 이 경로로 새 나갔다).
  빈 DB 에 전부 순서대로 적용해 보는 것이 가장 싼 확인이다
...
- **대역이 통과시킨 것은 통과한 게 아니다.** 같은 모양으로 세 번 깨졌다 — 합성 픽스처가 실제 SQL Server
  계획의 `StatementText`를 안 갖고 있었고(195절), 복합 키 테스트가 두 기종만 덮어 게시된 릴리스가 SQL
  Server에서 죽었고(200절), H2가 만든 스키마 때문에 마이그레이션 컬럼 정의가 틀려도 테스트가 통과했다
  (201절, V48의 `AUTO_INCREMENT`도 같은 자리). 대역을 쓰지 말라는 뜻이 아니라, **그 대역이 덮지 못하는
  축을 알고 그 축만 실물로 한 번 밟으라**는 뜻이다. 마이그레이션은 실제 PostgreSQL에, 기종별 경로는
  그 기종에, 파서는 실제 출력으로
- **테스트를 쓰는 것과 CI가 그것을 돌리는 것은 다른 일이다.** 게이트 환경변수로 막힌 테스트는 건너뛰고도
  초록불이 뜬다 — 초록불이 "확인했다"가 아니라 "아무것도 안 했다"를 뜻할 수 있다. 기종 전용 잡에 테스트를
  추가하면 결과 파일을 읽어 실제로 돌았는지 단언하는 검사도 함께 건다(#140)
