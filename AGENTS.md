...
- **버튼은 예외다(162절)** — 콘텐츠가 아니라 손이 닿는 조작 계층이라, 불투명한 표·카드 위에서도 유리로 둔다.
  다만 색 버튼에 흰 글자를 얹은 채 반투명하게 만들면 대비가 무너진다(primary 4.91 -> 4.26, danger 4.46 -> 4.02,
  accent는 불투명에서도 2.47로 원래 미달이었다). 색을 옅게 깔고 글자를 같은 계열 진한 색으로 뒤집어야
  4.5:1을 넘는다(primary 7.62 · accent 5.70 · danger 6.89 · dark만 흰 글자 14.97).
