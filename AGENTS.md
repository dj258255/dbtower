...
- 소스에 원시 NUL 바이트를 넣지 않는다 — 맵 키 구분자 같은 제어문자는 이스케이프(`\u0000`)로 적는다.
  한 바이트만 들어가도 git이 그 파일을 바이너리로 보고 diff·리뷰·grep에서 사라진다(154절, 규약 검사 7이 잡는다)
...
- 유리 재질(`--glass-*` 토큰)은 떠 있는 계층에만 쓴다 — 상단바·사이드바·세그먼트 탭·드롭다운·툴팁·모달.
  ...
  **뒤에 비칠 콘텐츠가 없으면 유리는 장식이다** — 로그인은 카드·흐림·광원 없이 단색 배경에 입력 한 묶음만 둔다(172절)
- **버튼은 채운 면 세 종류다(184절, 162절 개정)** — 기본(회색 #f2f4f6 + #333d4b)·주(하늘 #0a6aa8 + 흰 글자)·위험(옅은 빨강 + #b42335).
  유리 버튼은 표·카드 위에서 낡아 보였고, 옅은 색 유리 + 진한 글자는 무엇이 주 동작인지 약했다. 영역마다 주 버튼은 하나.
  색 버튼에 흰 글자를 얹을 때는 채운 면이어야 한다 — 반투명 면이면 대비가 무너진다(162절: primary 4.91 -> 4.26).
  같은 선택자를 파일 두 곳에서 칠하지 않는다 — 나중 규칙만 보이고 앞의 대비 계산이 통째로 무의미해진다
- 드롭다운은 네이티브 select 대신 `enhanceSelect`(패널을 body에 띄움)로 — 표·팝오버·overflow 칸 안에서 잘리지 않고 늘 아래로 연다.
  아래 공간이 모자라면 페이지를 먼저 올린다. 열자마자 생긴 스크롤로 패널을 닫지 않는다(184절: E2E와 사람 모두 목록이 즉시 닫혔다).
  macOS 네이티브 목록은 선택 항목을 버튼 위치에 맞추느라 위로 열린다
- `data-*` 속성에 제어문자(NUL 등)를 싣지 않는다 — HTML 파서가 대체 문자로 바꿔 JS에서 읽은 값이 원래 키와 달라진다(184절: 워크벤치 뷰 묶음이 펼쳐지지 않았다)
- 화면에 서버 응답 본문·드라이버 원문을 내보내지 않는다 — 공용 `api()`가 `{error, errorId}`를 나눠 사람 문장 + 오류 번호 앞 8자만 보인다(148·184절)
- 글자 대비는 4.5:1 이상을 눈대중이 아니라 계산으로 확인한다(보조 글자 #52697d: 흰 바탕 5.71, 배경 #eef7fd 5.27)
...
- 모니터링 탭은 "무엇을 하러 왔나"로 나눈다(184절) — 성능·진단·설정 비교(설정·스키마·파라미터)·변경 승인(리뷰 게이트·온라인 스키마 변경)·
...
- 문제 하나 = 이슈 하나, 영역 하나 = PR 하나, 변경 한 가지 = 커밋 하나. 여러 영역을 한 PR·큰 커밋에 몰지 않는다.
  여러 건을 묶어 볼 때는 하위 이슈 체크리스트만 있는 상위 이슈를 둔다
- 이슈·PR마다 `type:*`·`area:*` 라벨과 마일스톤을 단다(개인 저장소라 GitHub 네이티브 이슈 타입은 쓸 수 없다).
  마일스톤은 **릴리스 버전**(v1.5.0)이다. "UX 2차" 같은 작업 묶음은 마일스톤이 아니라 `track:*` 라벨과 상위 이슈로 둔다 — 마일스톤이 곧 릴리스 범위가 되게
- 릴리스는 묶어서 낸다. 작업을 끝낼 때마다 태그를 치지 않는다 — v1.3.0·v1.3.1·v1.4.0을 사흘 사이에 냈고 v1.5.0·v1.6.0도 2주 안에 잡혀,
  판마다 이미지·변경 이력·업그레이드 안내가 반복되고 "이번에 무엇이 바뀌었나"를 한 문장으로 말할 수 없었다.
  한 릴리스는 한 이야기로 묶이는 범위이고, 성격이 다른 큰 기능(대량 일괄 변경 같은)은 따로 둔다. 그 사이 변경은 main과
  CHANGELOG `[Unreleased]`에 쌓는다. 보안·데이터 손상 결함만 패치 버전(x.y.z)으로 바로 낸다
- 착수 전에 이슈에 예상(시간·일 단위)·예상 마감(날짜만, 예: 09-18)·완료 조건을 적고, 끝나면 실제 마감과 어긋난 이유를 채운다. 마감을 넘기면 원래 값을 지우지 않고 새 예상을 한 줄 더 적는다.
  없는 예상을 뒤늦게 만들어 넣지 않는다
- 같은 파일을 고치는 PR은 앞 PR 위에 쌓고 본문에 머지 순서를 적는다. **쌓인 PR을 머지할 때 head 브랜치를 바로 지우지 않는다** —
  base가 지워지면 GitHub가 뒤 PR을 닫는다(184절). 다음 PR의 base를 main으로 바꾼 뒤 머지하고, 브랜치는 마지막에 정리한다
- 화면이 바뀐 PR은 머지 전에 실제 브라우저로 확인한다 — 페이지 넘침 0·E2E 통과로는 안 보이는 결함이 있었다(181·184절)
